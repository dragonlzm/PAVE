...
### what we change is the loading of the model
### the feature loading
...
class EvalDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 video_folder,
                 video_feature_folder,
                 audio_feature_folder,
                 image_processor,
                 ):
        ...
        for ele in all_dialogs:
            ...
            curr_qa_dict = {'video_path': os.path.join(video_folder, curr_video_name),
                            'video_feat_path': os.path.join(video_feature_folder, curr_video_name.split('.')[0] + '.pt'),
                            'audio_feat_path': os.path.join(audio_feature_folder, curr_video_name.split('.')[0] + '.pt'),
                            'vid': vid,
    ...
    def __getitem__(self, i):
        ...
        curr_video_feat_path = all_ele_content['video_feat_path']
        curr_audio_feat_path = all_ele_content['audio_feat_path']
        ...
        # load the feature
        if os.path.exists(curr_video_feat_path):
            video_feature = torch.load(curr_video_feat_path, map_location=torch.device('cpu'))
            
        if os.path.exists(curr_audio_feat_path):
            audio_feature = torch.load(curr_audio_feat_path, map_location=torch.device('cpu'))
...
def eval_model(args):
    ...
    eval_dataset = EvalDataset(args.annotation_file,    # path to the mapping between the video id and the video path
                 args.video_folder,
                 args.video_feature_folder,
                 args.audio_feature_folder, # additional feature folder
                 image_processor,
    ...
    for ele in tqdm(eval_dataloader):
        ...
        # ipdb.set_trace()
        video_feat = ele['video_feature'][:, :, 1:,] # torch.Size([1, 56, 5, 1024])
        S = video_feat.shape[2]
        assert int(math.sqrt(S)) ** 2 == S # assert is a square
        W = H = int(math.sqrt(S))        
        video_feat = rearrange(video_feat, 'b t (h w) c ->b c t h w', h = H) # video_feat should be in the shape of (B, C, T, H, W)
        # print('fast_feat:', fast_feat.shape)
        video_feat = video_feat.half().cuda()
        feat_frame_nums = video_feat.shape[2]
        
        
        audio_feature = ele['audio_feature'].permute([0, 2, 1]).unsqueeze(dim=-1).unsqueeze(dim=-1).half().cuda() # (B, T, C) -> (B, C, T, H, W)
        audio_feat_frame_nums = audio_feature.shape[2]
        ...
        try:
            ...
            with torch.inference_mode():
                output_ids = model.generate(input_ids, 
                                            video_feats=video_feat, # (B,C,T,H,W)
                                            video_feat_fps=torch.tensor([1]).cuda(), # meaningless info
                                            feat_frame_nums=torch.tensor([feat_frame_nums]).cuda(),
                                            
                                            second_feats=audio_feature, # added for multi-sides
                                            second_feat_fps=torch.tensor([1]).cuda(), # meaningless info
                                            second_feat_frame_nums=torch.tensor([audio_feat_frame_nums]).cuda(),                                                 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video-feature-folder", type=str, default="")
    parser.add_argument("--audio-feature-folder", type=str, default="")
